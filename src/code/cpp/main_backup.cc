...
double GetRateOnSingleCircle(const vector<int> &cluster) {
  ...
  double denom = min(
    double(e_max * sum_consuming_rate) / (max_consuming_rate * to_sensor_rate),
...
double GetRateOnSingleCluster(vector<int> cluster) {
  ...
  while (start < (int)cluster.size()) {
    ...
    while (high - low > 1) {
      ...
      double bar = double(to_sensor_rate) / sum_consuming_rate - 1 - double(from_mc_rate) / to_mc_rate;
      ...
      if (numer > denom) {
        ...
        last_circle_time = denom + t_m + (denom * from_mc_rate + t_m * moving_rate) / to_mc_rate;
    ...
    // auto new_sum_consuming_rate = sum_consuming_rate + sensors[id].consuming_rate;
    // auto new_max_consuming_rate = max(max_consuming_rate, sensors[id].consuming_rate);
    ...
    // auto t_m = test.nearest_neighbor();        // run nearest neighbor function
    // double foo = double(t_m * (1 + double(moving_rate) / to_mc_rate));
    // double bar = double(to_sensor_rate) / new_sum_consuming_rate - 1 - double(from_mc_rate) / to_mc_rate;
    // double numer = foo / bar;
    
    // double denom = min(
    //   double(e_max * new_sum_consuming_rate) / (new_max_consuming_rate * to_sensor_rate),
    //   double(e_mc_max - t_m * moving_rate) / from_mc_rate
    // );
...
double GetRate(const vector<vector<int>> &clusters, vector<double> &rate_vector) {
  double res = 0.0;
  int cnt = 0;
  for (auto cluster : clusters) {
    auto rate = GetRateOnSingleCluster(cluster);
    res = max(res, rate);
...
vector<vector<int>> MoveClusters(vector<vector<int>> clusters, const vector<double> &rate_vector,
    bool use_rate_vector = true) {

  int u = 0;
  int v = 0;
  if (use_rate_vector) { 
    u = max_element(rate_vector.begin(), rate_vector.end()) - rate_vector.begin();
    if (clusters[u].size() == 1) {
      cerr << fixed << setprecision(5) << rate_vector[u] << endl;
      cout << clusters[u][0] << endl;
    }
    ...
    for (int i = 0; i < 5; i++) {
      ...
      for (int j = 0; j < (int)rate_vector.size(); j++) {
        if (find(min_id.begin(), min_id.end(), j) != min_id.end() || j == u) continue;
        if (cur == -1) cur = j;
        else {
...
bool CanArrange(int m) {
  ...
  if (m > 1 && rate > 0.99) {
    while (num_try < 3 && rate > 0.99) {
      ...
      for (int it = 0; it < 10 && rate > 0.99; ++it) {

...
class tree_node
{
    public:
        ...
        int set_city(shared_ptr<city>& city_in); // copies item_in into the node
        shared_ptr<city>& get_city(); // returns the item contained in the node
    protected:
        shared_ptr<city> tree_city;
        tree_node * left;
        tree_node * right;
};
...
class tree
{
    ...
    private:
        ...
        void traverse_and_build(tree_node * root, deque <shared_ptr<city>> & neighbor_list, int size);
